...
import com.sopt.korailtalk.domain.type.InstallmentType
...
@Composable
fun PaymentInstallmentBottomSheet(
    showInstallmentBottomSheet: Boolean,
    onInstallmentBottomSheetChange: () -> Unit
) {
    val sheetState = rememberModalBottomSheetState()
    var selectedOptionIndex by remember { mutableIntStateOf(0) }
    val options = InstallmentType.entries.toTypedArray()

    KorailBottomSheet(
        isOpenBottomSheet = showInstallmentBottomSheet,
        title = "할부기간",
        content = {
        ...
        onDismissRequest = onInstallmentBottomSheetChange
    )
}